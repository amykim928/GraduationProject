package com.example.closet_app.tracker
...
import com.example.graduateproject.env.BorderedText
...
class MultiBoxTracker(context: Context) {
    ...
    @Synchronized
    fun trackResults(results: List<YoloInterfaceClassfier.Recognition>, timestamp: Long) {
    ...
    private fun processResults(results: List<YoloInterfaceClassfier.Recognition>) {
        val rectsToTrack: MutableList<Pair<Float, YoloInterfaceClassfier.Recognition>> =
            LinkedList<Pair<Float, YoloInterfaceClassfier.Recognition>>()
        screenRects.clear()
        ...
        for (result in results) {
            ...
            rectsToTrack.add(Pair<Float, YoloInterfaceClassfier.Recognition>(result.confidence, result))
        }
